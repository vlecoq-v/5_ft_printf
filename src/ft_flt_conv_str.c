...
/*   Updated: 2019/03/04 15:20:35 by vlecoq-v         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_flt_tp_conv_str(t_conv *c)
{
	...
	if (c->prc && !c->prc_sz)
		fra = (fra + fra >= 10) ? fra + 10 : fra;
	else
		fra = fra % 10 > 5 ? fra / 10 + 1 : fra / 10;
	if (fra && (fra >= ft_pwr(10, pwr)))
...
int		ft_ll_tp_conv_str(t_conv *c)
{
	...
	if (!(dec = ft_itoa_b_f(fra, c)))
		return (0);
	if (!(ent = (c->arg_lf < 0.0 || 1 / c->arg_lf < 0)
		? ft_itoa_b((long long)c->arg_lf, 10, c, 1)
